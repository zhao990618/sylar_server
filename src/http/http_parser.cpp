...
namespace sylar
{
    namespace http{
        ...
        size_t HttpRequestParser::execute(char* data, size_t len) {
            /*
                解析请求(request)或应答(response)报文，并返回实际解析的报文长度，
                此长度只代表解析一个完整报文扫描的长度，
                并不表示接收完整报文成功，完整报文的判断需要依赖on_message_complete回调。
            */ 
           // offset 是已经解析了的长度 len表示data的长度就是需要解析的长度
            size_t offset = http_parser_execute(&m_parser, data, len, 0);
            // 如果只有4字节的处理量，但是data超过了4字节，那么就需要将剩余的数据重新转入的data中
